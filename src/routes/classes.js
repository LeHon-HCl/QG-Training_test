...
async function handleGetClasses(req, res) {
  ...
  try {
    const grade = req.query.grade ? parseInt(req.query.grade) : null // 三目运算：如果有年级查询参数，将其转换为整数；否则设为null
    const classes = await classService.getAllClasses(grade)
    sendJson(res, 200, classes)
  } catch (err) {
    console.error('获取所有班级失败:', err)
